...
AddTab("MM2", "🔪", function()
    ...
    AddToggle("Trigger Bot (auto-shoot)", Config.TriggerBot, function(v) Config.TriggerBot = v end)
    AddSlider("Trigger Range", 10, 100, Config.TriggerBotRange, function(v) Config.TriggerBotRange = v end)
    AddToggle("Aimbot V1 (Hold)", Config.AimbotV1, function(v) Config.AimbotV1 = v end)
    AddDropdown("Aim Part", {"Head","HumanoidRootPart","Torso"}, Config.AimbotPart, function(t) Config.AimbotPart = t end)
    AddToggle("Aim Silent V2 (Flick auto)", Config.AimbotV2, function(v) Config.AimbotV2 = v end)
...
AddTab("Settings", "⚙", function()
    AddButton("Destroy GUI", function()
        ...
        ChamsFolder:Destroy() ESPFolder:Destroy() RoleESPFolder:Destroy()
...
UserInput.JumpRequest:Connect(function()
    ...
    -- Speed Glitch: при прыжке дать скорость
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aimbot V1 (удержание правой кнопки)
        ...
        if Config.RoleESP then
            for _, Plr in ipairs(Players:GetPlayers()) do
                if Plr ~= LocalPlayer then
                    ...
                    elseif not bb and Plr.Character then
                        CreateRoleESP(Plr)
                    end
                end
            end
...
-- Trigger Bot (стреляет когда мурдерер в зоне видимости, без фов)
task.spawn(function()
    while task.wait(0.05) do
        if Config.TriggerBot then
            local murderer, head = CheckMurdererInSight(Config.TriggerBotRange)
            if murderer and head then
                -- Поворачиваем камеру на мурдерера и стреляем
...
-- Aimbot V2 (Silent - флик убийство)
task.spawn(function()
    while task.wait(0.05) do
                ...
                if char then
                    for _, tool in ipairs(char:GetChildren()) do
                        if tool:IsA("Tool") and (tool.Name == "Gun" or tool.Name == "Revolver") then
                            ...
                            pcall(function() tool:Activate() end)
                            task.wait()
                            Camera.CFrame = origCF -- вернуть камеру (флик)
                        end
                    end
                end
            end
...
-- Auto Pickup Gun V2 (без телепорта - просто подбирает если рядом)
task.spawn(function()
...
-- Farm Coins V2 (мгновенный телепорт)
task.spawn(function()
